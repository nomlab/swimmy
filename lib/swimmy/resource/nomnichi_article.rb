    ...
    module Resource
      class NomnichiArticle
        ...
        def initialize(id, user_id, title, perma_link, content,
                       published_on, approved, count, promote_headline, url, user_name = nil)
  
          @id, @user_id, @title, @perma_link, @content,
          @published_on, @approved, @count, @promote_headline,
          @url, @user_name =id, user_id, title, perma_link, content,
          published_on, approved, count, promote_headline, url, user_name
        end
  
        def user_name
          @user_name ||= user_name_from_permalink
        end
